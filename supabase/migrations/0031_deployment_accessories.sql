-- Capture accessories/battery identification handed over at deployment.
-- Columns nullable in DB (existing rows safe); "required" enforced in zod.
-- Recreate deployments_enriched so d.* re-expands to include the new columns
-- (Postgres freezes the column list at view creation).

ALTER TABLE deployments ADD COLUMN battery_number text;
ALTER TABLE deployments ADD COLUMN charger_cable_number text;
