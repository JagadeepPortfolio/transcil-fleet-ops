-- days_left should be blank for terminal deployments. Previously it computed
-- due_date - today regardless of status, so RETURNED/CANCELLED rows still showed
-- a days-left number. Now NULL unless ACTIVE/LOCKED (and not billing-exempt).
...
SELECT
  ...
  END AS pay_status,
  CASE
    WHEN d.status NOT IN ('ACTIVE', 'LOCKED') THEN NULL   -- terminal: no days left
